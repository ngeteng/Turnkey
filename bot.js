...
const TelegramReporter = require('./telegram.js');

dotenv.config();

// --- BAGIAN KONFIGURASI, LOGGER, DAN UTILITAS ---
const config = {
...
class EVMBot {
    ...
    // ================================================================
    // ===== MODIFIKASI UTAMA ADA DI FUNGSI DI BAWAH INI (executeTransactionBatch) =====
    // ================================================================
    async executeTransactionBatch() {
        ...
        // --- BAGIAN BARU: Mengambil Saldo Akhir untuk Laporan ---
        logger.info('Mengambil data saldo akhir untuk laporan...');
        const balancePromises = this.wallets.map(async (wallet) => {
            const balance = await this.provider.getBalance(wallet.address);
            const balanceInEth = parseFloat(ethers.formatEther(balance)).toFixed(5);
            return `<code>${utils.shortenAddress(wallet.address)}</code> | Sisa: <b>${balanceInEth} ETH</b>`;
        });
        
        const walletReportLines = await Promise.all(balancePromises);
        const walletDetails = walletReportLines.join('\n'); // Gabungkan setiap baris laporan
        // --- AKHIR BAGIAN BARU ---
...
💰 <b>Detail Saldo Akhir:</b>
${walletDetails}
        `;
        await this.reporter.sendMessage(summaryMessage.trim());
    }
